 ...
 *                        'image'         => 'https://github.com/Codeinwp/fog-lite-pro/blob/production/inc/admin/welcome-screen/img/zblackbeard.jpg?raw=true',
	...
	class TI_About_Page {
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				...
				$title = $count > 0 ? $this->page_name . '' : $this->page_name;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<a href="https://themeflux.com/" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
					...
					echo '<h3>' . esc_html( $free_pro['get_pro_theme_label'] ) . '</h3>';
					echo '<p>' . esc_html( $free_pro['get_pro_theme_details'] ) . '</p>';
					echo '<a href="' . esc_url( $free_pro['pro_theme_link'] ) . '" target="_blank" class="button button-primary button-hero">' . wp_kses_post( $free_pro['get_pro_theme_label'] ) . '</a>';
					echo '</div>';
					echo '</div>';
			...
			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {
				...
				wp_localize_script(
					'ti-about-page-js', 'tiAboutPageObject', array(
						...
						'activating_string'   => __( 'Activating', 'fog-lite-pro' ),
					)
				);
